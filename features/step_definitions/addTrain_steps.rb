...
When /^(?:|I )follow "([^"]*)"$/ do |new|
  click_link(new)
end

Then /^(?:|I )should be on (.+)$/ do |new|
  current_path = URI.parse(current_url).path
  if current_path.respond_to? :should
    current_path.should == path_to(new)
  else
    assert_equal path_to(new), current_path
  end
end

When /^(?:|I )fill in "([^"]*)" with "([^"]*)"$/ do |line, n|
  fill_in(line, :with => n)
end
...
When /^(?:|I )select "([^"]*)" from "([^"]*)"$/ do |brooklyn, borough|
  select(brooklyn, :from => borough)
end
